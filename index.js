...
var REPO_PROPOSALS_BASE_URL = GITHUB_BASE_URL + 'weso/shex-lite-evolution/blob/master/proposals'
...
function renderNav () {
  ...
  var checkboxes = [
    ...
    '.implemented', '.returnedForRevision', '.awaitingImplementation', '.rejected', '.withdrawn'
  ].map(function (state) {
...
function renderBody () {
  ...
  var proposalPresentationOrder = [
    '.awaitingReview', '.scheduledForReview', '.activeReview', '.accepted',
    '.acceptedWithRevisions', '.implemented', '.returnedForRevision', '.awaitingImplementation', '.rejected', '.withdrawn'
  ]
...
function renderSummary(summary) {
  return html('div', { className: 'summary proposal-detail' }, [
    html('div', { className: 'proposal-detail-label' }, 'Summary: '),
    html('div', { className: 'proposal-detail-value' }, [
      html('p', { className: 'p-summary'}, summary)
    ])
  ])
}
